...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    if (!res.ok) {
      console.error("Send message failed:", payload);
    }
      ...
      <div className="fixed bottom-0 left-0 right-0 px-4 pb-6 pt-3 bg-[#0a0a0f]/95 backdrop-blur-xl border-t border-white/[0.06]">
        {imagePreview && (
          <div className="relative w-20 h-20 mb-2">
            ...
            <button onClick={() => setImagePreview(null)} className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-black/80 flex items-center justify-center">
              <X className="w-3 h-3 text-white" />
            </button>
          </div>
        )}
        <div className="flex items-center gap-2">
          <button onClick={() => imageRef.current?.click()} className="w-10 h-10 rounded-2xl bg-white/[0.07] flex items-center justify-center shrink-0 hover:bg-white/10 transition">
            {uploading ? (
